...
//   const [state, setState] = React.useState([{ id: null, qty: null }]);
    ...
    <>
      ...
      <ul>
        {data.item.map((v, i) => {
          ...
          const onClickkPlus = () => {
            if (v.id === value.id) {
              value.push({ ...value, qty: v.qty + 1 });
            } else {
              value.push({ id: v.id, qty: v.qty + 1 });
            }
            return value.qty;
          };
          //버튼이 클릭이 되었을 때 push하지만 id가 같은게 없다면 새로 push
          ...
          return (
            <div key={v.id}>
              ...
              <FontAwesomeIcon className="numOption" icon={faPlus} onClick={onClickkPlus}/>
            </div>
          );
        })}
      </ul>
